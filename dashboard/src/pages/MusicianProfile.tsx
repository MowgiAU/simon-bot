        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (err.response?.status === 404) {
                    // Profile doesn't exist yet, initialize local state
                    setProfile({
                        bio: '',
                        spotifyUrl: '',
                        soundcloudUrl: '',
                ...
                const genresRes = await axios.get('/api/musician/genres', { withCredentials: true });
                setAllGenres(genresRes.data);
            } finally {
                setLoading(false);
            }
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: spacing.md }}>
                                {profile?.genres.map(g => (
                                    <div key={g.id} style={{ 
                                        ...
                                        backgroundColor: colors.primary, padding: '4px 8px', borderRadius: '16px', fontSize: '0.85rem' 
                                    }}>
                                        {g.name}
                            ...
                            <div style={{ position: 'relative' }}>
                                <select 
                                    ...
                                    style={{ 
                                        ...
                                        border: 'none', borderRadius: borderRadius.sm, padding: spacing.sm, color: colors.textPrimary 
                                    }}
                                >
                                    <option value="">Add a genre...</option>
                                    {allGenres
                                        .filter(g => !profile?.genres.some(pg => pg.id === g.id))
                                        .map(g => (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
                                ...
                                <button 
                                    ...
                                    style={{ 
                                        ...
                                        borderRadius: borderRadius.sm, padding: spacing.sm, color: colors.textSecondary, cursor: 'pointer',
