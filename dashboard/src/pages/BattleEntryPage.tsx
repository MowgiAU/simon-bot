...
import {
    ...
    SkipBack, SkipForward, Scale, Swords, Trophy
} from 'lucide-react';
import { useLocation, useNavigate, Link } from 'react-router-dom';
import { CommentSection } from '../components/CommentSection';
import { AddToPlaylistModal } from '../components/AddToPlaylistModal';
import { ReportButton } from '../components/ReportButton';
import { ArrangementViewer, ArrangementData } from '../components/ArrangementViewer';
...
export const BattleEntryPage: React.FC = () => {
    ...
    const [entry, setEntry] = useState<BattleEntry | null>(null);
    ...
    const [lyricsSyncState, setLyricsSyncState] = useState<Array<{ time: number; text: string }>>([]);
    ...
    useEffect(() => {
        const entryId = pathname.split('/').filter(Boolean).pop();
        if (!entryId) return;
    ...
    const toggleFavourite = async () => {
        if (!track || !user) { showToast('Login to favourite tracks', 'error'); return; }
        try {
        ...
        } catch { showToast('Login to favourite tracks', 'error'); }
    };

    const toggleRepost = async () => {
        if (!track || !user) { showToast('Login to repost tracks', 'error'); return; }
        ...
        } catch { showToast('Login to repost tracks', 'error'); }
    };

    const openEditMode = () => {
        if (!track) return;
        setEditForm({
            title: track.title || '', description: track.description || '', artist: track.artist || '',
            album: track.album || '', year: track.year?.toString() || '', bpm: track.bpm?.toString() || '',
            key: track.key || '', allowAudioDownload: track.allowAudioDownload ?? true,
            allowProjectDownload: track.allowProjectDownload ?? true, license: track.license || 'all-rights-reserved',
        });
        setSelectedTrackGenres(track.genres?.map(g => g.genre.id) || []);
        setEditAudioFile(null); setEditArtworkFile(null); setEditProjectFile(null);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            navigate(entry ? `/battles/${entry.battle.id}` : '/battles');
        } catch (e: any) {
    ...
    useEffect(() => {
        ...
        if (!cues || cues.length === 0 || player.currentTrack?.id !== track?.id) { setActiveLyricIdx(-1); return; }
        const ct = player.currentTime;
        let idx = -1;
        for (let i = 0; i < cues.length; i++) { if (cues[i].time <= ct) idx = i; else break; }
    ...
    const openLyricsEdit = () => {
        if (!track) return;
        const rawLines = track.lyricsSync?.length ? track.lyricsSync.map(c => c.text).join('\n') : (track.lyrics || '');
        setLyricsText(rawLines);
        setLyricsSyncState(track.lyricsSync ? [...track.lyricsSync] : []);
        setLyricsTab('write'); setLyricsEditOpen(true);
    };

    const handleLyricsTabSwitch = (tab: 'write' | 'sync') => {
        if (tab === 'sync') {
            const lines = lyricsText.split('\n');
            setLyricsSyncState(prev => lines.map((text, i) => ({ time: prev[i]?.time ?? 0, text })));
        }
        setLyricsTab(tab);
    };
    ...
    const tapSyncTime = (idx: number) => {
        ...
        setLyricsSyncState(prev => prev.map((l, i) => i === idx ? { ...l, time: ct } : l));
    };

    const formatSyncTime = (t: number) => { const m = Math.floor(t / 60); const s = (t % 60).toFixed(1).padStart(4, '0'); return `${m}:${s}`; };
    const parseSyncTime = (val: string): number => { const match = val.match(/^(\d+):(\d+(?:\.\d+)?)$/); if (!match) return 0; return parseInt(match[1]) * 60 + parseFloat(match[2]); };

    const handleSaveLyrics = async () => {
        if (!track) return;
        setLyricsSaving(true);
        try {
            const hasSync = lyricsSyncState.length > 0 && lyricsSyncState.some(c => c.time > 0);
            const payload = { lyrics: lyricsText.trim() || null, lyricsSync: hasSync ? lyricsSyncState : null };
            const res = await axios.put(`/api/musician/tracks/${track.id}/lyrics`, payload, { withCredentials: true });
            setEntry(prev => prev && prev.track ? { ...prev, track: { ...prev.track, lyrics: res.data.lyrics, lyricsSync: res.data.lyricsSync } } : prev);
            setLyricsEditOpen(false);
            showToast('Lyrics saved!', 'success');
        } catch (e: any) { showToast(e.response?.data?.error || 'Failed to save lyrics', 'error'); }
    ...
    if (loading) return (
        <DiscoveryLayout activeTab="discovery">
            <div style={{ display: 'flex', justifyContent: 'center', padding: '100px', color: colors.textSecondary }}>Loading entry...</div>
        </DiscoveryLayout>
    );
    ...
    if (error || !entry) return (
        <DiscoveryLayout activeTab="discovery">
            <div style={{ textAlign: 'center', padding: '100px' }}>
                <h2 style={{ color: '#ff4444' }}>{error || 'Entry not found'}</h2>
                <div style={{ marginTop: spacing.xl }}>
                    <button onClick={() => window.history.back()} style={{ backgroundColor: 'transparent', color: colors.primary, border: `1px solid ${colors.primary}`, padding: '8px 16px', borderRadius: borderRadius.md, cursor: 'pointer' }}>← Go Back</button>
                </div>
            </div>
        </DiscoveryLayout>
    );
    ...
    return (
        <DiscoveryLayout activeTab="discovery">
            <div style={{ maxWidth: '1300px', margin: '0 auto', padding: isMobile ? '16px' : spacing.xl }}>
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                                {track.projectFileUrl && (track.allowProjectDownload ?? true) && (
                                    <>
                                        <button onClick={() => user ? setFlpConfirmOpen(true) : (window.location.href = '/login')}
                        ...
                        {track.arrangement.tracks.some(t => t.clips.length > 0) && (
                            <div style={{ padding: isMobile ? '16px 20px' : '20px 28px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                <MemoizedArrangement track={track} player={player} isPlaying={isPlaying} zoom={zoom} setZoom={setZoom} />
                            </div>
                        )}
                        ...
                        {track.arrangement.projectInfo && (track.arrangement.projectInfo.plugins.length > 0 || track.arrangement.projectInfo.samples.length > 0) && (
                            <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                <button onClick={() => setPluginsSamplesOpen(!pluginsSamplesOpen)}
                                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '14px 20px' : '14px 28px', background: 'none', border: 'none', cursor: 'pointer', color: colors.textSecondary }}>
                                            ...
                                            {track.arrangement.projectInfo!.plugins.length > 0 && (
                                                <div>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                                                        <Zap size={15} color={colors.primary} />
                                                        <span style={{ fontSize: '12px', fontWeight: 700, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Plugins ({track.arrangement.projectInfo!.plugins.length})</span>
                                                    </div>
                                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                                        {track.arrangement.projectInfo!.plugins.map((plugin, i) => (
                                                            <span key={i} style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 500, backgroundColor: 'rgba(242,123,19,0.08)', border: '1px solid rgba(242,123,19,0.15)', color: '#F0A060' }}>{plugin}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            )}
                                            {track.arrangement.projectInfo!.samples.length > 0 && (
                                                <div>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                                                        ...
                                                        <span style={{ fontSize: '12px', fontWeight: 700, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Samples ({track.arrangement.projectInfo!.samples.length})</span>
                                                    </div>
                                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                                        {(expandedSamples ? track.arrangement.projectInfo!.samples : track.arrangement.projectInfo!.samples.slice(0, 12)).map((sample, i) => (
                                                            <span key={i} style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 500, backgroundColor: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.15)', color: '#C4A8FF', maxWidth: '260px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sample}</span>
                                                        ))}
                                                    </div>
                ...
                {/* Track details (no FLP) — audio download only */}
                {!track.arrangement && track.allowAudioDownload && (
                    ...
                    <div style={{ marginBottom: '24px', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', backgroundColor: 'rgba(255,255,255,0.02)' }}>
                        <div style={{ padding: '18px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                {track.lyricsSync && track.lyricsSync.length > 0 && (
                                    <span style={{ fontSize: '11px', fontWeight: 600, padding: '3px 8px', borderRadius: '20px', background: `${colors.primary}22`, border: `1px solid ${colors.primary}44`, color: colors.primary }}>Synced</span>
                                )}
                            </div>
                            {canEdit && (
                                <button onClick={openLyricsEdit} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 14px', borderRadius: '8px', cursor: 'pointer', border: `1px solid ${colors.primary}44`, background: `${colors.primary}11`, color: colors.primary, fontSize: '13px', fontWeight: 600 }}>
                        ...
                        <div style={{ padding: '20px 24px' }}>
                            {!track.lyrics && (!track.lyricsSync || track.lyricsSync.length === 0) && (
                                <p style={{ margin: 0, color: colors.textTertiary, fontSize: '0.9rem', fontStyle: 'italic' }}>No lyrics added yet.</p>
                            )}
                            {track.lyricsSync && track.lyricsSync.length > 0 ? (
                                <div ref={lyricsContainerRef} style={{ maxHeight: '360px', overflowY: 'auto', paddingRight: '8px' }}>
                                    {(track.lyricsSync as Array<{ time: number; text: string }>).map((cue, i) => (
                                        <div key={i} ref={i === activeLyricIdx ? activeLineRef : undefined}
                                            onClick={() => { if (player.currentTrack?.id === track.id) seek(cue.time); else setTrack(track, [track]); }}
                                            style={{ padding: '7px 0', cursor: 'pointer', fontSize: activeLyricIdx === i ? '1.1rem' : '0.95rem', fontWeight: activeLyricIdx === i ? 700 : 400, color: activeLyricIdx === i ? colors.primary : colors.textSecondary, transition: 'all 0.25s ease', lineHeight: 1.5, userSelect: 'none' as const }}>
                                            {cue.text || <span style={{ opacity: 0.3 }}>♪</span>}
                                        </div>
                                    ))}
                                </div>
                            ) : track.lyrics ? (
                                <pre style={{ margin: 0, color: colors.textSecondary, fontFamily: 'inherit', whiteSpace: 'pre-wrap', lineHeight: 1.8, fontSize: '0.93rem' }}>{track.lyrics}</pre>
                            ) : null}
                        </div>
                    </div>
                )}
                ...
                {/* ═══ LYRICS EDITOR MODAL ═══ */}
                {lyricsEditOpen && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 10000, backgroundColor: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                        <div style={{ backgroundColor: colors.surface, borderRadius: borderRadius.lg, border: '1px solid rgba(255,255,255,0.1)', width: '100%', maxWidth: '680px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                                ...
                                <button onClick={() => setLyricsEditOpen(false)} style={{ background: 'none', border: 'none', color: colors.textSecondary, cursor: 'pointer' }}><X size={22} /></button>
                            </div>
                            <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                                {(['write', 'sync'] as const).map(t => (
                                    <button key={t} onClick={() => handleLyricsTabSwitch(t)}
                                        style={{ flex: 1, padding: '12px', border: 'none', cursor: 'pointer', background: lyricsTab === t ? `${colors.primary}18` : 'transparent', color: lyricsTab === t ? colors.primary : colors.textSecondary, fontWeight: lyricsTab === t ? 700 : 400, fontSize: '0.9rem', borderBottom: lyricsTab === t ? `2px solid ${colors.primary}` : '2px solid transparent' }}>
                                        {t === 'write' ? '✏ Write' : '⏱ Sync'}
                                    </button>
                                ))}
                            </div>
                            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
                                {lyricsTab === 'write' && (
                                    <div>
                                        <p style={{ margin: '0 0 12px', fontSize: '13px', color: colors.textSecondary }}>Paste or type your lyrics. Each line will become a sync-able cue.</p>
                                        <textarea value={lyricsText} onChange={e => setLyricsText(e.target.value)}
                                            placeholder={'Verse 1\nYour lyrics here...\n\nChorus\nSing along...'} rows={18}
                                            style={{ width: '100%', boxSizing: 'border-box' as const, backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, padding: '14px', color: colors.textPrimary, fontSize: '14px', lineHeight: 1.7, resize: 'vertical' as const, fontFamily: 'inherit', outline: 'none' }} />
                                    </div>
                                )}
                                {lyricsTab === 'sync' && (
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: borderRadius.md, border: '1px solid rgba(255,255,255,0.07)' }}>
                                            <button onClick={() => seek(Math.max(0, (player.currentTrack?.id === track?.id ? player.currentTime : 0) - 5))} title="Skip back 5s" style={{ background: 'none', border: 'none', color: colors.textSecondary, cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center' }}><SkipBack size={18} /></button>
                                            <button onClick={() => player.currentTrack?.id === track?.id ? togglePlay() : setTrack(track!, [track!])} style={{ width: '36px', height: '36px', borderRadius: '50%', flexShrink: 0, background: colors.primary, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 2px 10px ${colors.primary}55` }}>
                                                {player.currentTrack?.id === track?.id && player.isPlaying ? <Pause size={16} fill="white" color="white" /> : <Play size={16} fill="white" color="white" style={{ marginLeft: '2px' }} />}
                                            </button>
                                            <button onClick={() => seek((player.currentTrack?.id === track?.id ? player.currentTime : 0) + 5)} title="Skip forward 5s" style={{ background: 'none', border: 'none', color: colors.textSecondary, cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center' }}><SkipForward size={18} /></button>
                                            <div style={{ width: '1px', height: '20px', backgroundColor: 'rgba(255,255,255,0.08)', margin: '0 2px' }} />
                                            <span style={{ fontSize: '12px', color: colors.textSecondary, flex: 1 }}>Click <strong style={{ color: colors.textPrimary }}>Tap</strong> on a line to stamp the current time.</span>
                                            <span style={{ fontSize: '14px', fontWeight: 700, color: colors.primary, minWidth: 56, textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums', fontFamily: 'monospace' }}>{formatSyncTime(player.currentTrack?.id === track?.id ? player.currentTime : 0)}</span>
                                        </div>
                                        {lyricsSyncState.length === 0 && <p style={{ color: colors.textTertiary, fontStyle: 'italic', fontSize: '13px' }}>No lines yet — go back to Write and add your lyrics first.</p>}
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                            {lyricsSyncState.map((cue, i) => (
                                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                    <input type="text" value={formatSyncTime(cue.time)}
                                                        onChange={e => { const t = parseSyncTime(e.target.value); if (!isNaN(t)) setLyricsSyncState(prev => prev.map((l, j) => j === i ? { ...l, time: t } : l)); }}
                                                        style={{ width: '72px', flexShrink: 0, textAlign: 'center' as const, padding: '6px 8px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.sm, color: colors.primary, fontSize: '12px', fontFamily: 'monospace', outline: 'none' }} />
                                                    <span style={{ flex: 1, fontSize: '14px', color: cue.text ? colors.textPrimary : colors.textTertiary, fontStyle: cue.text ? 'normal' : 'italic', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{cue.text || '(empty line)'}</span>
                                                    <button onClick={() => tapSyncTime(i)} style={{ flexShrink: 0, padding: '5px 12px', borderRadius: borderRadius.sm, border: `1px solid ${colors.primary}55`, background: `${colors.primary}18`, color: colors.primary, fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>Tap</button>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            <div style={{ padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                <button onClick={() => setLyricsEditOpen(false)} style={{ padding: '9px 20px', borderRadius: borderRadius.md, border: '1px solid rgba(255,255,255,0.15)', background: 'transparent', color: colors.textSecondary, cursor: 'pointer', fontSize: '14px' }}>Cancel</button>
                                {(track.lyrics || (track.lyricsSync && track.lyricsSync.length > 0)) && (
                                    <button onClick={async () => {
                                        setLyricsSaving(true);
                                        try {
                                            await axios.put(`/api/musician/tracks/${track.id}/lyrics`, { lyrics: null, lyricsSync: null }, { withCredentials: true });
                                            setEntry(prev => prev && prev.track ? { ...prev, track: { ...prev.track, lyrics: null, lyricsSync: null } } : prev);
                                            setLyricsEditOpen(false); showToast('Lyrics removed', 'success');
                                        } catch { showToast('Failed to remove lyrics', 'error'); } finally { setLyricsSaving(false); }
                                    }} style={{ padding: '9px 20px', borderRadius: borderRadius.md, border: '1px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.1)', color: '#EF4444', cursor: 'pointer', fontSize: '14px' }}>Remove Lyrics</button>
                                )}
                                <button onClick={handleSaveLyrics} disabled={lyricsSaving}
                                    style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 20px', borderRadius: borderRadius.md, border: 'none', background: colors.primary, color: 'white', cursor: lyricsSaving ? 'not-allowed' : 'pointer', fontSize: '14px', fontWeight: 700, opacity: lyricsSaving ? 0.7 : 1 }}>
                ...
                {/* ═══ EDIT MODAL ═══ */}
                {editing && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
                        <div style={{ backgroundColor: colors.surface, borderRadius: borderRadius.lg, border: '1px solid rgba(255,255,255,0.1)', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto', padding: '32px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                <div>
                                    ...
                                    <input type="text" value={editForm.title} onChange={e => setEditForm(f => ({ ...f, title: e.target.value }))} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box' }} />
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                    <div>
                                        ...
                                        <input type="text" value={editForm.artist} onChange={e => setEditForm(f => ({ ...f, artist: e.target.value }))} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box' }} />
                                    </div>
                                    <div>
                                        <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', color: colors.textSecondary, fontWeight: 600 }}>Album</label>
                                        <input type="text" value={editForm.album} onChange={e => setEditForm(f => ({ ...f, album: e.target.value }))} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box' }} />
                                    </div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                                    <div>
                                        ...
                                        <input type="number" value={editForm.year} onChange={e => setEditForm(f => ({ ...f, year: e.target.value }))} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box' }} />
                                    </div>
                                    <div>
                                        <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', color: colors.textSecondary, fontWeight: 600 }}>BPM</label>
                                        <input type="number" value={editForm.bpm} onChange={e => setEditForm(f => ({ ...f, bpm: e.target.value }))} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box' }} />
                                    </div>
                                    <div>
                                        <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', color: colors.textSecondary, fontWeight: 600 }}>Key</label>
                                        <input type="text" value={editForm.key} placeholder="e.g. C minor" onChange={e => setEditForm(f => ({ ...f, key: e.target.value }))} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box' }} />
                                    </div>
                                </div>
                                <div>
                                    <label style={{ display: 'block', marginBottom: '6px', fontSize: '0.85rem', color: colors.textSecondary, fontWeight: 600 }}>Description</label>
                                    <textarea value={editForm.description} onChange={e => setEditForm(f => ({ ...f, description: e.target.value }))} rows={3} style={{ width: '100%', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: borderRadius.md, color: 'white', fontSize: '0.95rem', outline: 'none', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                                </div>
                                {/* Genre Tags */}
                                <div>
                                    <label style={{ marginBottom: '6px', fontSize: '0.85rem', color: colors.textSecondary, fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}><Tag size={14} /> Genre Tags</label>
                                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '8px' }}>
                                        {selectedTrackGenres.map(gid => {
                                            const g = allGenres.find((ag: any) => ag.id === gid);
                                            return g ? (
                                        ...
                                        {selectedTrackGenres.length === 0 && <span style={{ fontSize: '0.85rem', color: colors.textSecondary, fontStyle: 'italic' }}>No genres selected</span>}
                                    </div>
                                    <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                                        <input type="text" placeholder="Search genres..." value={genreSearchTerm} onChange={e => setGenreSearchTerm(e.target.value)}
                                        ...
                                        <select value="" onChange={e => { if (e.target.value && !selectedTrackGenres.includes(e.target.value)) { setSelectedTrackGenres(prev => [...prev, e.target.value]); setGenreSearchTerm(''); } }}
                                                    ...
                                                    {parentsWithChildren.map((parent: any) => {
                                                        ...
                                                        return (
                                                            <optgroup key={parent.id} label={parent.name} style={{ backgroundColor: '#1A1E2E' }}>
                                                                {children.map((c: any) => (<option key={c.id} value={c.id} style={{ backgroundColor: '#1A1E2E', color: 'white' }}>{c.name}</option>))}
                                                            </optgroup>
                                                        );
                                                    })}
                                                </>);
                                ...
                                <div style={{ marginTop: '4px', padding: '12px', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: borderRadius.md }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.9rem' }}>
                                            <input type="checkbox" checked={editForm.allowAudioDownload} onChange={e => setEditForm(f => ({ ...f, allowAudioDownload: e.target.checked }))} />
                                            Public: Allow Audio Download
                                        </label>
                                        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.9rem' }}>
                                            <input type="checkbox" checked={editForm.allowProjectDownload} onChange={e => setEditForm(f => ({ ...f, allowProjectDownload: e.target.checked }))} />
                                ...
                                <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '16px', marginTop: '4px' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                        ...
                                        <div>
                                            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', padding: '10px 14px', backgroundColor: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.15)', borderRadius: borderRadius.md, color: colors.textSecondary, fontSize: '0.9rem' }}>
                                                <Upload size={16} />
                                                {editProjectFile ? editProjectFile.name : 'Replace .flp project file'}
                            ...
                            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '24px' }}>
                                <button onClick={() => setEditing(false)} style={{ padding: '10px 20px', borderRadius: borderRadius.md, border: '1px solid rgba(255,255,255,0.15)', backgroundColor: 'transparent', color: colors.textSecondary, cursor: 'pointer', fontSize: '0.95rem' }}>Cancel</button>
                                <button onClick={handleSaveEdit} disabled={saving}
                                    style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 24px', borderRadius: borderRadius.md, backgroundColor: colors.primary, color: 'white', border: 'none', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.95rem', opacity: saving ? 0.7 : 1 }}>
            ...
            <ConfirmModal open={deleteConfirmOpen} title="Delete Track"
