                ...
                <div style={{ flex: 1, overflowY: 'auto', paddingBottom: player.currentTrack ? 90 : 0 }}>
                    ...
                    {hero && (
                        <section style={{ position: 'relative', width: '100%', height: 400, overflow: 'hidden' }}>
                            ...
                            {hero.track.coverUrl && (
                                <img src={hero.track.coverUrl} alt="" referrerPolicy="no-referrer" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.15 }} />
                            )}
                            <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(to top, ${BG} 0%, rgba(15,19,29,0.6) 60%, rgba(15,19,29,0.3) 100%)` }} />
                            <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 40px', textAlign: 'center' }}>
                                ...
                                {hero.track.coverUrl && (
                                    <div onClick={() => playEntry(hero)} style={{ width: 72, height: 72, borderRadius: 10, overflow: 'hidden', marginBottom: 14, boxShadow: '0 8px 32px rgba(0,0,0,0.6)', cursor: 'pointer', flexShrink: 0 }}>
                                        <img src={hero.track.coverUrl} alt="" referrerPolicy="no-referrer" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                    </div>
                                )}
                                <h1 style={{ margin: '0 0 6px', fontSize: 48, fontWeight: 900, color: '#fff', letterSpacing: '-0.03em', maxWidth: 700, lineHeight: 1.05 }}>{hero.track.title}</h1>
                                <Link to={`/profile/${hero.track.profile?.username}`} style={{ color: SUB, textDecoration: 'none', fontSize: 17, fontWeight: 600, marginBottom: 16 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 28, marginBottom: 14 }}>
                                    <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: SUB, fontSize: 13 }}><Play size={14} /> {fmtNum(hero.track.playCount)} plays</span>
                                    <button onClick={() => playEntry(hero)} style={{ width: 44, height: 44, borderRadius: '50%', background: PRIMARY, border: 'none', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: `0 0 20px ${PRIMARY}55`, flexShrink: 0 }}>
                                        {playingId === hero.track.id ? <Pause size={20} fill="#fff" color="#fff" /> : <Play size={20} fill="#fff" color="#fff" style={{ marginLeft: 2 }} />}
                                    </button>
                                    <span style={{ display: 'flex', alignItems: 'center', gap: 6, color: SUB, fontSize: 13 }}><Repeat2 size={14} /> {fmtNum(hero.playsInPeriod)} this period</span>
                                </div>
                                <div style={{ height: 30, display: 'flex', alignItems: 'center', gap: 1.5, maxWidth: 420, width: '100%' }}>
                    ...
                    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 32px 40px', boxSizing: 'border-box' }}>
                        ...
                        <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                                <div>
                                    <h2 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: TEXT }}>Global Charts</h2>
